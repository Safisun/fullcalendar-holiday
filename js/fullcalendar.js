...
// A defined `.eventProps`, even when empty, indicates that an event should be created.
    ...
    DayGrid.mixin({
        ...
        fgSegHtml: function (seg, disableResizing) {
            ...
            //自定义格式
            if (event.star) {
                startext = event.star.toLowerCase();
                if (startext == "a") {
                    starhtml = '<span class="fc-star-a"></span>';
                } else if (startext == "b") {
                    starhtml = '<span class="fc-star-b"></span>';
                } else if (startext == "c") {
                    starhtml = '<span class="fc-star-c"></span>';
                } else if (startext == "d") {
                    starhtml = '<span class="fc-star-d"></span>';
                }
            }
debugger
            titleHtml =
                ...
                '</span>';
            if (event.type) {
                starhtml = '<span class="anndays"  ></span>';
                titleHtml =
                    '<span class="fc-title" rel="annpopover">' +
                    (htmlEscape(event.title || '') || '&nbsp;') + // we always want one line of height
                    '</span>';
            }
...
// We mixin DayTable, even though there is only a single row of days
    ...
    TimeGrid.mixin({
        ...
        fgSegHtml: function (seg, disableResizing) {
            ...
            classes.unshift('fc-time-grid-event', 'fc-v-event', 'fc-event-w');
        ...
        buildGotoAnchorHtml: function (gotoOptions, attrs, innerHtml) {
            ...
            if (!forceOff && this.opt('navLinks')) {
                ...
                if (attrs) {
                    ...
                    var holiday = loadholiday(moment(date).format("YYYY-MM-DD"));
                    if (cnDay == '正月')
                        cnDay = '春节';
                    switch (cnMonDay) {
                        case '正月初一':
                    ...
                    if ((date.getMonth() == 0) && (date.getDate() == 1)) cnDay = "元旦";
                    if ((date.getMonth() == 1) && (date.getDate() == 14 )) cnDay = "情人节";
                    if ((date.getMonth() == 2) && (date.getDate() == 8)) cnDay = "妇女节";
                    if ((date.getMonth() == 2) && (date.getDate() == 12)) cnDay = "植树节";
                    if ((date.getMonth() == 3) && (date.getDate() == 1)) cnDay = "愚人节";
                    if ((date.getMonth() == 4) && (date.getDate() == 1)) cnDay = "劳动节";
                    if ((date.getMonth() == 5) && (date.getDate() == 1)) cnDay = "儿童节";
                    if ((date.getMonth() == 7) && (date.getDate() == 1)) cnDay = "建军节";
                    if ((date.getMonth() == 8) && (date.getDate() == 10)) cnDay = "教师节";
                    if ((date.getMonth() == 9) && (date.getDate() == 1)) cnDay = "国庆节";
                    if ((date.getMonth() == 11) && (date.getDate() == 24)) cnDay = "平安夜";
                    if ((date.getMonth() == 11) && (date.getDate() == 25)) cnDay = "圣诞节";
                    return '<span' + attrs + '>' +
                        innerHtml +
                        '</span><span class="solarday">' + cnDay + '</span><span class="holiday">' + holiday + '</span>';
                } else {
                    return '<span' + attrs + '>' +
                        innerHtml +
                        '</span>';
...
// from largest to smallest
    ...
    var ListViewGrid = Grid.extend({
        ...
        fgSegHtml: function (seg) {
            ...
            if (event.type) {
                icon = "<span class='anndays'></span>";
                timeHtml = "";
            } else {
                icon = '<img src="http://help.ufile.ucloud.com.cn/web/calendar_' + event.star.toLowerCase() + '.png">';
            }
            ...
            return '<tr class="' + classes.join(' ') + '">' +
                ...
                '<a' + (url ? ' href="' + htmlEscape(url) + '"' : '') + '><span class="fc-title">' +
                htmlEscape(seg.event.title || '') +
                '</span></a>' +
                '</td>' +
                '<td class="ispriv">' + (event.ismeet == 1 ? '<img src="http://help.ufile.ucloud.com.cn/web/meeticon.png">' : "") + (event.ispriv == 1 ? '<img src="http://help.ufile.ucloud.com.cn/web/privicon.png">' : "") +
                '</td>' +
                '</tr>';
        }
 ...
 * @returns {string}
 */
function loadholiday(date) {
    var holidayhtml = '';
    for (var i = 0; i < rootVM.holiday.length; i++) {
        if (date == rootVM.holiday[i].id) {
            if (rootVM.holiday[i].type == 1) {
                holidayhtml = "假";
                break;
            } else if (rootVM.holiday[i].type == 2) {
                holidayhtml = "休";
                break;
            } else if (rootVM.holiday[i].type == 3) {
                holidayhtml = "班";
                break;
            } else {
                holidayhtml = "";
